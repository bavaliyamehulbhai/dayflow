  ...
  return (
    <MobileBottomSheet
      ...
      headerAction={
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button 
            onClick={() => { onSave({ title, content, color, tags: tags.split(',').map(t => t.trim()).filter(Boolean) }, false); onClose(); }} 
            style={{ 
          ...
          <button 
            onClick={() => isDeleting ? onSave(null, false, true) : (setIsDeleting(true), setTimeout(() => setIsDeleting(false), 3000))} 
            style={{ 
              background: isDeleting ? 'rgba(255, 100, 100, 0.2)' : 'rgba(255,255,255,0.03)', 
              borderRadius: '12px', 
              padding: '8px', 
              border: isDeleting ? '1px solid var(--red)' : '1px solid rgba(255,255,255,0.05)', 
    ...
    >
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div className="liquid-glass-input" style={{ padding: '0 16px', display: 'flex', alignItems: 'center', height: 64, gap: 12 }}>
           <div style={{ padding: '6px', background: `${color}22`, borderRadius: 8, display: 'flex', flexShrink: 0 }}>
        ...
        <div className="liquid-glass-input" style={{ padding: '20px', flex: 1 }}>
          <textarea
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            <div style={{ fontSize: 9, fontWeight: 900, color: 'var(--muted)', letterSpacing: 1 }}>SELECT FREQUENCY:</div>
              ...
              {NOTE_COLORS.map(c => (
                <motion.button 
                  ...
                  style={{ 
                    ...
                    borderRadius: '50%', 
                    background: c.value, 
                    border: color === c.value ? '2px solid white' : '1px solid rgba(255,255,255,0.2)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {isSyncing ? <Activity size={12} className="aura-float" style={{ color: color }} /> : <Clock size={12} style={{ color: 'var(--muted)' }} />}
            <div style={{ fontSize: 9, fontWeight: 900, color: isSyncing ? color : 'var(--muted)', letterSpacing: 1.5, textTransform: 'uppercase' }}>
...
export default function NotesPage() {
  ...
  const NoteCard = React.memo(({ note }) => (
    <motion.div
      ...
      style={{ 
        ...
        padding: isMobile ? '20px' : '28px 36px', borderRadius: 32, cursor: 'pointer',
    ...
    >
      ...
      <div 
        ...
        style={{ flex: 1, display: 'flex', alignItems: 'center', gap: isMobile ? 16 : 28, minWidth: 0 }}
      >
        <div style={{ width: isMobile ? 48 : 64, height: isMobile ? 48 : 64, borderRadius: 22, background: `${note.color}15`, border: `1px solid ${note.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
          <FileText size={isMobile ? 18 : 26} style={{ color: note.color, filter: `drop-shadow(0 0 12px ${note.color})` }} />
          <div className="aura-pulse" style={{ position: 'absolute', inset: 0, borderRadius: 22, background: note.color, opacity: 0.1 }} />
        </div>
        ...
        {note.isPinned && (
          <div style={{ width: 32, height: 32, borderRadius: 12, background: 'var(--accent)22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
  ...
  return (
    <div className="responsive-container" style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden' }}>
      ...
      <AuraOrb color="rgba(0, 242, 254, 0.12)" size="500px" top="60%" left="60%" delay={4} />
          ...
          <div>
            <div className="page-title flex items-center gap-4">
              <div className="auth-logo-icon aura-float" style={{ width: isMobile ? 40 : 54, height: isMobile ? 40 : 54, marginBottom: 0, background: 'var(--grad-premium)' }}>
                <FileText size={isMobile ? 20 : 28} color="white" strokeWidth={2.5} fill="white" />
              </div>
              <h1 style={{ 
              ...
              }}>Cognitive Archive</h1>
            </div>
            <p className="page-subtitle" style={{ fontSize: 'var(--fs-sm)', opacity: 0.8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', marginTop: 12 }}>Neural snapshots & memory nodes</p>
          </div>
          <MagneticButton className="auth-button hide-mobile glow-on-hover" onClick={() => createMutation.mutate({ title: '', content: '' })} style={{ width: 'auto', padding: '0 32px', height: 60, borderRadius: 24, fontSize: 14, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 1 }}>
            <div className="btn-glint" />
            <Plus size={22} style={{ marginRight: 10 }} strokeWidth={3} /> MANIFEST ESSENCE
          </MagneticButton>
        </div>
